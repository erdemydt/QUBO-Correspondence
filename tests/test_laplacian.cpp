// Cotangent Laplacian and lumped mass. These invariants are what the spectral
// basis depends on: if rows do not sum to zero, or mass does not match surface
// area, the eigenfunctions are wrong in ways that are hard to diagnose later.
...
// A unit square as two triangles: small enough to check by hand, so the right
// place to pin the sign convention.
Mesh unit_square() {
  Mesh m;
  m.V.resize(4, 3);
  m.V << 0, 0, 0,
...
void test_analytic() {
  ...
  // Both triangles are right isoceles: the right angle gives cot = 0, the two
  // 45-degree angles cot = 1. Edge 0-2 is opposite the right angle in both, so
  // its weight is 0.
  ...
  // Sign convention: positive semi-definite, so the diagonal is positive.
...
void test_invariants(const char* spec) {
  ...
  // L annihilates constants -- the single most informative check, since almost
  // every assembly bug breaks it.
  ...
  // Mass matrix must be positive and account for exactly the surface area.
  ...
  // Sampled PSD check: the Rayleigh quotient must be non-negative anywhere.
...
void test_alignment_consistency() {
  ...
  // Shared connectivity means structurally identical operators, even though the
  // geometry and therefore the weights differ.
