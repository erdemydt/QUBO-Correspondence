// Encodes facts about the data on disk that later stages rely on, notably the
// index gaps that force scanning over generated filenames.
...
void test_groups(const DatasetRegistry& reg) {
  ...
  // Not vertex-aligned with the horse class (15974 vs 19248), so grouping it
  // there would make evaluation assume false ground truth.
...
void test_resolution(const DatasetRegistry& reg) {
  ...
  // By stem: an index alone is ambiguous (cat0, dog0, wolf0, ...).
...
void test_gaps(const DatasetRegistry& reg) {
  ...
  // A formula would return paths for these and fail later, confusingly.
  ...
  // horse10 must sort after horse7, not after horse1.
  const auto horses = reg.meshes("tosca", "horse");
